...
fn calculate_simple_material(board: &Board) -> i32 {
    let piece_values = [
        0, 10, 20, 100, 20, 25, 30, 15  // Soldier, Jester, Commander, Paladin, Guard, Dragon, Ballista
    ];
...
fn main() {
    ...
    println!("Initial position: {} (positive = White advantage)", initial_material);
    ...
    if let Ok(board_after_capture) = game.apply_move_copy(arx_engine::game::Move {
        ...
        println!("  Material: {} (positive = White advantage)", material_after);
        println!("  Change: {} (negative = Black gained material)", material_after - initial_material);
    ...
    if let Ok(board_after_stack) = game.apply_move_copy(arx_engine::game::Move {
        ...
        println!("  Material: {} (positive = White advantage)", material_after);
        println!("  Change: {} (should be near 0, just stacking own pieces)", material_after - initial_material);
