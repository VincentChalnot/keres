import {Config, Board, PotentialMove, Move} from '../models/types.ts';
import {decodeBoardFromBinary, encodeBoardToBinary, decodePotentialMove, encodeMove} from '../utils/boardUtils.ts';
...
export class GameAPI {
    ...
    async getMinimaxMove(board: Board): Promise<Move> {
        const binary = encodeBoardToBinary(board);
        const response = await fetch(`${this.config.backendUrl}/minimax-move`, {
            method: 'POST',
            headers: {'Content-Type': 'application/octet-stream'},
            body: binary as BodyInit,
        });
